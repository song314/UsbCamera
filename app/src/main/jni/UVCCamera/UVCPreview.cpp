...
void UVCPreview::addPreviewFrame(uvc_frame_t *frame) {
    ...
    int ret = 0;
    //in frame type: MJPEG, data bytes unfixed, size is 1280*720
    uvc_frame_t *yuyv_frame = get_frame(frame->width * frame->height * 2); //for YUYV
    ret = uvc_mjpeg2yuyv(frame, yuyv_frame);  //MJPEG-->YUYV
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
    ...
    uvc_frame_t *frame_RGBX = NULL; //uvc_frame_t *frame_mjpeg = NULL;
    ...
    if (LIKELY(!result)) {
        ...
        if (frameMode) {
            ...
            for ( ; LIKELY(isRunning()) ; ) {
                frame_RGBX = waitPreviewFrame();
                if (LIKELY(frame_RGBX) && (frame_RGBX->actual_bytes == frame_RGBX->width * frame_RGBX->height * 4)) {
                    if (LIKELY(!result)) {
                        draw_preview_one(frame_RGBX, &mPreviewWindow, uvc_any2rgbx, 4);
                        addCaptureFrame(frame_RGBX);
                    } else {
...
uvc_frame_t *UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window, convFunc_t convert_func, int pixcelBytes) {
    ...
    pthread_mutex_unlock(&preview_mutex);
    if (LIKELY(b)) {
        //Now in frame format is RGBX, echo directly
        pthread_mutex_lock(&preview_mutex);
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
    ENTER();
    //Now in frame format is RGBX
    if (LIKELY(frame)) {
        uvc_frame_t *YUYV_frame = get_frame(frame->width * frame->height * 2);  //for YUYV
        uvc_frame_t *IYUV_frame = get_frame(frame->width * frame->height * 3 / 2);  //for IYUV420SP
        if (mFrameCallbackObj) {
...
//                    IYUV_frame = frame;
            ...
            jobject buf = env->NewDirectByteBuffer(IYUV_frame->data, frame->width * frame->height * 3 / 2);
